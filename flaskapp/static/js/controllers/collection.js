App.controller('collectionController', ['$scope','$http','$compile','currentProduct', function($scope,$http,$compile,currentProduct) {


$scope.collectionPageInit = function()
{
	$scope.imageCollection= [];
	for (var curImageCounter = 1; curImageCounter <= 2; curImageCounter++) 
	{
		 $scope.imageCollection.push('purse'.concat(curImageCounter).concat('.jpg'));
	}
}

$scope.collectionProductHit = function(curProductArg)
{
	currentProduct.curProduct=curProductArg;
}

//$scope.noImages=[1,2,3];
//$scope.collectionImages=
//str1.concat(str2);
//$scope.no.forEach((element, index, array)
